...
class Table_Columns {
	...
	public function get_hidden_columns() {
		...
		if ( $this->args->filters ) {
			$hidden = preg_replace( '/$/', '_hfilter', array_map( [ Columns_Util::class, 'unprefix_column' ], $this->args->filters ) );
		}
	...
	public function get_column_heading_class( $index, $column ) {
		...
		return implode( ' ', apply_filters( 'posts_table_header_class_' . Columns_Util::unprefix_column( $column ), $class ) );
	}
	...
	public function get_column_priority( $index, $column ) {
		...
		$priority = apply_filters( 'posts_table_column_priority_' . Columns_Util::unprefix_column( $column ), $priority );
	...
	public function get_column_width( $index, $column ) {
		$width = apply_filters( 'posts_table_column_width_' . Columns_Util::unprefix_column( $column ), '' );
	...
	public function is_click_filterable( $column ) {
		...
		$click_filterable = apply_filters( 'posts_table_column_click_filterable_' . Columns_Util::unprefix_column( $column ), $click_filterable );
	...
	public function is_searchable( $column ) {
		...
		if ( $searchable ) {
			$searchable = apply_filters( 'posts_table_column_searchable', $searchable, Columns_Util::unprefix_column( $column ) );
			$searchable = apply_filters( 'posts_table_column_searchable_' . Columns_Util::unprefix_column( $column ), $searchable );
		}
	...
	public function is_sortable( $column ) {
		...
		if ( $sortable ) {
			$sortable = apply_filters( 'posts_table_column_sortable', $sortable, Columns_Util::unprefix_column( $column ) );
			$sortable = apply_filters( 'posts_table_column_sortable_' . Columns_Util::unprefix_column( $column ), $sortable );
		}
