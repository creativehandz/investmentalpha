...
class Posts_Table {
	...
	public function update( array $args ) {
		...
		$post_selection_args   = apply_filters( 'posts_table_update_post_selection_args', [ 'post_type', 'status', 'year', 'month', 'day', 'category',
			'tag', 'term', 'cf', 'author', 'exclude', 'include', 'exclude_category', 'exclude_term', 'search_term' ] );
	...
	private function add_post_data( $column, $key, $post ) {
		...
		$data = apply_filters( 'posts_table_cell_data_' . Columns_Util::unprefix_column( $column ), $data, $post );
	...
	private function add_attributes() {
		...
		if ( $custom_class = apply_filters( 'posts_table_custom_class', '', $this ) ) {
