...
class WpeCommon extends WpePlugin_common {
	...
	public function wpe_adminbar() {
        ...
        $has_manage_options = $user->has_cap( 'manage_options' );
	...
	if( $wl = $this->is_whitelabel() )
	{
		...
		if ( filter_input( INPUT_GET, 'page', FILTER_SANITIZE_STRING ) === 'wpengine-common' ) {
	...
	public function upload_space_load() {
		...
		// Extra validation in case $dirsize is not false or array
		if ( ! is_array( $dirsize ) && $dirsize != false ) {
	...
	public function set_wpe_auth_cookie() {
		...
		$cookie_value = hash("sha256", 'wpe_auth_salty_dog|'.WPE_APIKEY);
    ...
    public function php_version_cookie( $version_string = '7.2' ) {
        ...
        setcookie( $this::PHP_VERSION_COOKIE_NAME, $php_version_hash, $cookie_expire, $cookie_path, '', true, true );
    }
